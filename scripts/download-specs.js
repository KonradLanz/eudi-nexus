...
const HISTORY_FILE   = path.join(DOWNLOAD_PATH, '_download_history.json');
...
async function printSummary(runResults) {
  ...
  for (const e of withDocx) {
    if (!e.filePath) continue; // guard: old entries may lack filePath
...
// ── Run history ────────────────────────────────────────────────────────────────────
//
// Appends a compact snapshot of every run to _download_history.json.
// This lets you detect regressions: if a spec downloaded fine last Tuesday
// but fails today, it shows up as a new `failed` entry with a timestamp.
//
// Structure:
//   [ { ts, flags, counts: { success, redownloaded, skipped, stopped, failed, noDownload },
//       failed: [{etsiNumber, reason}], noDownload: [{etsiNumber}] }, … ]
//
// Entries are append-only — never modified — so you always have the full audit trail.
// The file lives next to _download_results.json and is .gitignore'd (local only).

async function appendHistory(runResults) {
  let history = [];
  try { history = JSON.parse(await fs.readFile(HISTORY_FILE, 'utf-8')); } catch { /* first run */ }

  const flags = [
    PUBLISHED_ONLY     ? '--published-only'      : null,
    HEADERS_ONLY       ? '--headers-only'         : null,
    FORCE_UPDATE_CHECK ? '--force-update-check'   : null,
    FORCE_DOWNLOAD     ? '--force-download'        : null,
    DOCX_ONLY          ? '--docx-only'             : null,
    LIMIT              ? `--limit=${LIMIT}`         : null,
  ].filter(Boolean);

  // Detect regressions: items that previously succeeded but now failed
  let previousResults = null;
  try { previousResults = JSON.parse(await fs.readFile(RESULTS_FILE, 'utf-8')); } catch { /* ok */ }
  const prevSuccessNums = new Set([
    ...((previousResults?.success      ?? []).map(e => e.etsiNumber)),
    ...((previousResults?.redownloaded ?? []).map(e => e.etsiNumber)),
  ]);
  const regressions = (runResults.failed ?? []).filter(e => prevSuccessNums.has(e.etsiNumber));
  ...
  const entry = {
    ...
    hasAuth:     HAS_AUTH,
    counts: {
      success:     (runResults.success      ?? []).length,
      redownloaded:(runResults.redownloaded  ?? []).length,
      skipped:     (runResults.skipped       ?? []).length,
      stopped:     (runResults.stopped       ?? []).length,
      failed:      (runResults.failed        ?? []).length,
      noDownload:  (runResults.noDownload    ?? []).length,
    },
    // Only persist the problem items — successes are already in _download_results.json
    failed:      (runResults.failed     ?? []).map(e => ({ etsiNumber: e.etsiNumber, reason: e.reason })),
    noDownload:  (runResults.noDownload ?? []).map(e => ({ etsiNumber: e.etsiNumber })),
    regressions: regressions.map(e => ({ etsiNumber: e.etsiNumber, reason: e.reason })),
  };

  history.push(entry);

  // Warn in console if regressions detected
  if (regressions.length) {
    console.log(`\n⚠️  Regressions detected: ${regressions.length} spec(s) that previously downloaded successfully now fail:`);
    for (const r of regressions) console.log(`      ❌ ${r.etsiNumber}: ${r.reason}`);
    console.log(`   Check ${path.relative(PROJECT_ROOT, HISTORY_FILE)} for full audit trail.`);
  }
